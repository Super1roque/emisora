...
export default function RadioPlayer() {
  ...
  // Si llegan con ?song=ID, saltar a esa canción y activar filtro
  useEffect(() => {
    ...
    if (idx === -1) return;
    setCurrentIdx(idx);
    setQuery(cleanName(playlist[idx].name));
    setSharedSong(true);
  }, [playlist]);
  ...
  return (
    <div style={{ maxWidth: 480, margin: '0 auto', padding: '2rem 1rem 4rem', minHeight: '100vh' }}>
          ...
          {current && current.type !== 'commercial' && (
            <a
              ...
              style={{
                ...
                marginTop: '0.75rem', padding: '0.35rem 0.85rem', borderRadius: 50,
                background: '#25d36622', border: '1px solid #25d36655',
                color: '#25d366', fontSize: '0.78rem', fontWeight: 600, textDecoration: 'none',
              }}
            >
      ...
      {sharedSong && (
        <button
          onClick={() => { setQuery(''); setSharedSong(false); window.history.pushState({}, '', '/'); }}
          style={{
            display: 'flex', alignItems: 'center', gap: '0.4rem',
            marginBottom: '0.75rem', padding: '0.45rem 0.9rem', borderRadius: 50,
            background: 'var(--surface)', border: '1px solid var(--border)',
            color: 'var(--muted)', fontSize: '0.82rem', cursor: 'pointer', fontWeight: 600,
          }}
        >
